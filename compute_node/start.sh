...
echo "[3/6] Starting socat tunnels..."
...
# 4. Test connectivity
echo "[4/6] Testing connectivity to control plane..."
FAIL=0
...
# 5. Login to HuggingFace for faster downloads
if [ -n "${HF_TOKEN:-}" ]; then
  echo "[5/6] Logging into HuggingFace..."
  hf auth login --token "$HF_TOKEN"
  echo "  HF authenticated"
else
  echo "[5/6] No HF_TOKEN set — skipping HF login (slower downloads)"
fi

# 6. Start Celery worker
echo "[6/6] Starting Celery worker (solo pool — no fork, CUDA-safe)..."
